...
async def fetch_prices_from_yahoo(keyword: str) -> list[int]:
    """
    ヤフオクの落札履歴（終了品相場）から、直近2週間以内のデータに絞り込み、かつ「まとめ売り」「セット」等を除外した単体の落札価格リストを取得します。
    """
                    ...
                    if "落札" in parts:
                        ...
                        for part in parts[idx:]:
                            ...
                            if match:
                                ...
                                # 年を推定（基本は現在年、月が現在月より大きく現在月が1月付近の場合は前年とみなす）
                                item_year = now.year
...
async def fetch_prices_from_mercari(keyword: str) -> list[int]:
    """
    メルカリの検索結果（売り切れ品）から、新着順で直近の取引に絞り込み、かつ「まとめ売り」「セット」等を除外した単体の販売価格リストを取得します。
    """
    # 出品が新しい順（新着順）かつ売り切れ品で検索
    url = f"https://jp.mercari.com/search?keyword={keyword}&status=sold_out&sort=created_time&order=desc"
    headers = {
...
async def scan_figure(file: UploadFile = File(...)):
    ...
    try:
        ...
        prompt = (
            ...
            "mercari_priceとyahoo_priceには、あなたの知識データベースを元に、【直近2週間以内程度の中古市場での取引実績】に基づいた、かつ【まとめ売り・セット売り・詰め合わせ・限定豪華版同梱セットなどの高額ケースを除外した、このフィギュア単体（中古・箱あり・良品）】のリアルタイムに近い大体の参考相場価格（カンマなしの数値、例: 4500）を推測して出力してください。\n"
